...
interface AppDao {
    ...
    fun getAllMissions(): Flow<List<com.example.data.model.TaskMissionEntity>>

    @Query("SELECT * FROM task_missions WHERE id = :id LIMIT 1")
    suspend fun getMissionById(id: String): com.example.data.model.TaskMissionEntity?
    ...
    fun getAllGameMinerItems(): Flow<List<com.example.data.model.GameMinerItemEntity>>

    @Query("SELECT * FROM game_miner_items")
    suspend fun getAllGameMinerItemsSync(): List<com.example.data.model.GameMinerItemEntity>

    @Query("SELECT * FROM game_miner_items WHERE isPlacedInRoom = 1 ORDER BY placedSlotIndex ASC")
    fun getPlacedMinerItems(): Flow<List<com.example.data.model.GameMinerItemEntity>>

    @Query("SELECT * FROM game_miner_items WHERE isPlacedInRoom = 1 ORDER BY placedSlotIndex ASC")
    suspend fun getPlacedMinerItemsSync(): List<com.example.data.model.GameMinerItemEntity>

    @Query("SELECT * FROM game_miner_items WHERE id = :id LIMIT 1")
    suspend fun getGameMinerItemById(id: String): com.example.data.model.GameMinerItemEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertGameMinerItems(items: List<com.example.data.model.GameMinerItemEntity>)

    @Update
    suspend fun updateGameMinerItem(item: com.example.data.model.GameMinerItemEntity)

    // Game Room State
    @Query("SELECT * FROM game_room_state WHERE id = :id LIMIT 1")
    fun getGameRoomState(id: String = "default_room"): Flow<com.example.data.model.GameRoomStateEntity?>

    @Query("SELECT * FROM game_room_state WHERE id = :id LIMIT 1")
    suspend fun getGameRoomStateSync(id: String = "default_room"): com.example.data.model.GameRoomStateEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertGameRoomState(state: com.example.data.model.GameRoomStateEntity)

    @Update
    suspend fun updateGameRoomState(state: com.example.data.model.GameRoomStateEntity)
}
